...
import { GitDiffManager, type GitDiffResult } from '../git/DiffManager';
import type { CreateExecutionDiffData, ExecutionDiff } from '../../infrastructure/database';
...
interface ExecutionContext {
  ...
  prompt?: string;
  skipCheckpointAutoCommit?: boolean;
}

interface NextExecutionOptions {
  skipCheckpointAutoCommit?: boolean;
}

export class ExecutionTracker extends EventEmitter {
  private activeExecutions: Map<string, ExecutionContext> = new Map();
  private nextExecutionOptions: Map<string, NextExecutionOptions> = new Map();

  constructor(
    private sessionManager: import('../session').SessionManager,
  ...
  async startExecution(sessionId: string, worktreePath: string, promptMarkerId?: number, prompt?: string): Promise<void> {
    try {
      ...
      const beforeCommitHash = await this.gitDiffManager.getCurrentCommitHash(worktreePath, sessionId);
      this.logger?.verbose(`Starting from commit: ${beforeCommitHash}`);
      
      const options = this.nextExecutionOptions.get(sessionId);
      if (options) this.nextExecutionOptions.delete(sessionId);
      ...
      const context: ExecutionContext = {
        ...
        executionSequence,
        prompt,
        skipCheckpointAutoCommit: options?.skipCheckpointAutoCommit
      };
  ...
  setNextExecutionOptions(sessionId: string, options: NextExecutionOptions): void {
    this.nextExecutionOptions.set(sessionId, options);
  }
  ...
  async endExecution(sessionId: string): Promise<void> {
    try {
      ...
      // No auto-commit: capture diffs against current working tree state.
      const afterCommitHash = await this.gitDiffManager.getCurrentCommitHash(context.worktreePath, sessionId);
      ...
      if (afterCommitHash === context.beforeCommitHash) {
        // No changes at all
        executionDiff = await this.gitDiffManager.captureWorkingDirectoryDiff(context.worktreePath, sessionId);
      ...
      if (afterCommitHash !== context.beforeCommitHash && afterCommitHash !== 'UNCOMMITTED') {
        try {
          commitMessage = await this.gitDiffManager.getCommitSubject(context.worktreePath, afterCommitHash, sessionId);
